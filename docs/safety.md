...
Hook ownership is content- and topology-sensitive, not marker-based. `ao hooks status`
asks Git for the effective path, preserves foreign/ambiguous/symlinked/tracked content,
and labels exact generated bytes `behavior unverified`: static intent does not prove
that Git executed a hook. Scoped shared/external bodies also have deliberately fail-open
family routing; backlog #59 owns the runtime probe. Install, uninstall, and remove
preflight the complete mutation set and refuse all eligible shared/external or
global/system-configured mutations unless a person supplies `--allow-shared-hooks`.
The flag authorizes the location, never foreign or protected content. Pre-push remains
informational rather than a doctor alarm because a repository may intentionally retain
its own custom push hook.
