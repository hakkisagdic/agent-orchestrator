...
def test_profile_writes_role_blocks_and_keeps_existing(project):
    ...
    cfg = json.load(open(os.path.join(root, ".ao", "config.json")))
    ...
    json.dump(cfg, open(os.path.join(root, ".ao", "config.json"), "w"))
    assert cli._apply_profile(root, args) == []
    assert json.load(open(os.path.join(root, ".ao", "config.json")))["implementer"]["model"] == "custom"
